...
use wdk_sys::{
    _MODE::KernelMode,
    DISPATCHER_HEADER, DRIVER_OBJECT, HANDLE, OBJ_KERNEL_HANDLE, PETHREAD, PKTHREAD,
...
impl AltSyscalls {
    ...
    pub fn initialise_for_system(driver: &mut DRIVER_OBJECT) {
        ...
        //
        let driver_base = match get_module_base_and_sz("scil_driver.sys") {
        ...
        if rva_offset_callback > u32::MAX as _ {
            println!(
                "[scil] [-] Offset calculation very wrong? Offset: {:#x}",
                rva_offset_callback
            );
            return;
        }
        ...
        let kernel_service_descriptor_table = match lookup_global_table_address(driver) {
            ...
            Err(_) => {
                println!("[scil] [-] Failed to find kernel table");
                return;
            }
        };
    ...
    fn walk_active_processes_and_set_bits(
        ...
        if current_process.is_null() {
            println!("[scil] [-] current_process was NULL");
            return;
        }
                        ...
                        match thread_to_process_name(pe_thread as *mut _) {
                            Ok(current_process_name) => {
                                for needle in proc_vec.into_iter() {
                                    ...
                                    {
                                        println!(
                                            "[scil] [+] Process name found for alt syscalls: {}",
                                            needle
                                        );
                            ...
                            Err(e) => {
                                println!(
                                    "[scil] [-] Unable to get process name to set alt syscall bits on targeted process. {:?}",
                                    e
                                );
...
fn lookup_global_table_address(_driver: &DRIVER_OBJECT) -> Result<*mut c_void, DriverError> {
    let module = match get_module_base_and_sz("ntoskrnl.exe") {
        Ok(k) => k,
        Err(e) => {
            println!("[scil] [-] Unable to get kernel base address. {:?}", e);
